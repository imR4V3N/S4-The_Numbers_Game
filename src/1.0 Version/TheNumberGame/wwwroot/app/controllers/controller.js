...
app.controller("controller", function ($scope, $http, $interval) {
    ...
    $scope.sendCombinaison = function (combinaison) {
        // Verify combination
        $http.get(`${baseUrl}/game/verifCombinaison/`, { params: { combinaison: combinaison, listNumber: JSON.stringify($scope.listNumber) } }).then(function (response) {
            ...
            if (reponse_verification == 1) {
                ...
                $http.get(`${baseUrl}/game/getResultCombinaison/`, { params: { combinaison: combinaison } }).then(function (response) {
